...
def main(args):
    ...
    i_episode = 0  # 初始化i_episode

    with open("results_area_test.txt", 'a') as file:
        file.write("\n")
    ...
    while i_episode < args.max_episodes:
        ...
        for present_step in range(args.max_step):  # 循环无人机移动
            if whether_done:
                break
            # if present_step % 10 == 0:
            #     print("step %6d" % present_step)
            ...
            actions = []  # 动作列表
            # 直接去找最大的，大概是多少呢？
            # 贪心的基础上，将贪心目标作为最大对应点，然后结合附近位置进行训练
            ...
            # 将无人机移动对环境造成的影响进行更新。主要是更新的每个无人机储存的所有环境信息
            whether_done = env.update_state1()
            if present_step == args.max_step - 1:
                whether_done = True
            if whether_done:
                # 如果结束了，计算最终的reward与普通计算reward有差异
            ...
            greedy_state = env.get_greedy_rew()
...
if __name__ == '__main__':
    ...
    # test是强化学习的算法，greedy是寻找覆盖区域不确定和最大的位置的算法，greedy2是寻找最大不确定度位置的算法
    parser.add_argument('--test_mode', default="test", type=str, help='test/greedy/random/greedy2')
    # 环境相关基本要素
    ...
    parser.add_argument('--agents_num', default=5, type=int, help='无人机的数量')
    ...
    parser.add_argument('--greedy3_L', default=2, type=str,
                        help="滚动贪心搜索的预测长度")
